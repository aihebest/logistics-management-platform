...
const nav = [
  ...
  { to: '/vehicles',     label: 'Vehicles',       icon: 'M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0M3 7h18M3 7l2-4h14l2 4M3 7v7a2 2 0 002 2h1m12 0h1a2 2 0 002-2V7', roles: [] },
  { to: '/trips',        label: 'Trip Requests',  icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', roles: [] },
  ...
  { to: '/fuel',         label: 'Fuel Logs',      icon: 'M13 10V3L4 14h7v7l9-11h-7z', roles: [] },
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const visibleNav = nav.filter(item =>
    item.roles.length === 0 || hasRole(...(item.roles as Parameters<typeof hasRole>))
  )

  const Sidebar = () => (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-200">
        <img src="/desicon-logo.svg" alt="Desicon" className="w-9 h-9 flex-shrink-0" />
        <div className="leading-tight">
          <span className="font-bold text-gray-900 text-sm block">Desicon Engineering</span>
          <span className="text-xs text-gray-400">Logistics Platform</span>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 px-2 py-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500">
              {roles.length > 0
                ? roles[0]
                : <span className="italic text-gray-400">Loading role…</span>}
            </p>
          </div>
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col lg:pl-64">
        ...
        <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
          ...
          <Link to="/notifications" className="relative p-2 text-gray-400 hover:text-gray-600 rounded-md">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
